...
struct LockGuard {
    ...
    explicit LockGuard (std::mutex* mutex) : _std_mutex(mutex) {
        mutex->lock( );
    }
...
struct FileDescriptorWrapper {
    ...
    FileDescriptorWrapper (FileDescriptorWrapper&& other) noexcept {
        swap(std::move(other));
    }
    ...
    [[nodiscard]] const std::string& failMsg ( ) const noexcept {
        return failReason;
    }

    void setAutoRemove (bool removeOnClose = true) {
        autoRemove = removeOnClose;
    }
...
struct SharedMemoryDescriptorWrapper : public FileDescriptorWrapper {
    SharedMemoryDescriptorWrapper (const char* regionName, int opts, mode_t access) {
        fd = ::shm_open(regionName, opts, access);
    }
};
...
struct PthreadMutexAttrWrapper {
    ...
    operator pthread_mutexattr_t ( ) const {
        return mutexAttr;
    }
    ...
    const pthread_mutexattr_t* operator& ( ) const {
        return &mutexAttr;
    }
...
int close(wbr::sys::FileDescriptorWrapper&) = delete;
int close(const wbr::sys::socket_t&)        = delete;
int fclose(const wbr::sys::FileWrapper&)    = delete;
