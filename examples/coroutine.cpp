...
template<typename T>
generator<T> generateNumbersGeneric (T start, T end) {
    for ( T i = start; i <= end; ++i ) {
        co_yield i;
    }
}
...
generator<std::string, std::string_view, std::string_view> tokenizeEscapedGeneric (std::string_view input, std::string_view dividerChars = " ", char escapeChar = '\\') {
    auto is_quote   = [] (char c) { return c == '\'' || c == '"'; };
    auto is_divider = [dividerChars] (char c) { return dividerChars.find(c) != std::string_view::npos; };
    auto is_escape  = [escapeChar] (char c) { return c == escapeChar; };
